...
def recognize_date(date_):
    ...
    elif re.search('\d{2}[年]\d{1,2}[月]\d{1,2}[日号]', date_):
        temp = re.findall('\d{2}[年]\d{1,2}[月]\d{1,2}[日号]', date_)[0]
        dates_ = re.split('[年月日号]', temp)
        year_ = '20'+dates_[0]
        month_ = dates_[1]
        days_ = dates_[2]
        new_dates = datetime.date(int(year_), int(month_), int(days_))
        result.append(str(new_dates))
    ...
    # 纯月份
        ...
        elif '下半旬' in temp[1] or '下旬' in temp[1]:
            if int(temp[0]) != 2:
                dates_ = datetime.date(datetime.date.today().year, int(temp[0]), 15)
            else:
                dates_ = datetime.date(datetime.date.today().year, int(temp[0]), 14)
        result.append(str(dates_))
    ...
    # 上个月
    elif re.search('[前上]+[个]月', date_) or re.search('最近一个月', date_):
        if re.findall('[前上]+[个]月', date_):
            word = re.findall('[前上]+[个]月', date_)[0]
            times = word.count('上') + word.count('前')
            today = datetime.datetime.today()
            datime_delta = datetime.timedelta(days=32 * times)
            ...
            result.append(str(new_year))
    # 前一二三个月
    elif re.search('[前上][一二两三]个?月', date_):
        temp = re.findall('[前上][一二两三]个?月', date_)[0]
        new_year = None
        if '一' in temp:
            today = datetime.datetime.today()
            datime_delta = datetime.timedelta(days=32)
            new_year = (today - datime_delta).date().replace(day=1)
        elif '二' in temp or '两' in temp:
            today = datetime.datetime.today()
            datime_delta = datetime.timedelta(days=64)
            new_year = (today - datime_delta).date().replace(day=1)
        elif '三' in temp:
            today = datetime.datetime.today()
    ...
    elif re.search('[后下][一二两三]个?月', date_):
        temp = re.findall('[后下][一二两三]个?月', date_)[0]
        new_year = None
        if '一' in temp:
            today = datetime.datetime.today()
            datime_delta = datetime.timedelta(days=32)
    ...
    # 上周,这周,下周
        ...
        today = datetime.datetime.today()
        days = (int(datetime.date.today().strftime('%w')) - 1) % 7 + 7
        ...
        today = datetime.datetime.today()
        days = (int(datetime.date.today().strftime('%w')) - 1) % 7
        datime_delta = datetime.timedelta(days=days)
        result.append(str((today - datime_delta).date()))
    # 中文加月份
    ...
    # 上月,下月的各种
    elif re.search('上个?月[初末底]?', date_):
        today = datetime.datetime.today().date()
        temp = re.findall('上个?月[初末底]?', date_)[0]
        if '末' in temp or '底' in temp:
            new_date = datetime.date(datetime.date.today().year, today.month, 1) - datetime.timedelta(days=1)
        else:
            if today.month - 1 <= 0:
                new_date = datetime.date(datetime.date.today().year - 1, 12, 1)
            else:
                new_date = datetime.date(datetime.date.today().year, today.month, 1)
        result.append(str(new_date))
        ...
        if '末' in temp or '底' in temp:
            if today.month + 2 < 12:
                ...
                new_date = datetime.date(datetime.date.today().year + 1, today.month + 2 % 12, 1) - datetime.timedelta(
                    days=1)
        else:
            if today.month + 1 <= 12:
